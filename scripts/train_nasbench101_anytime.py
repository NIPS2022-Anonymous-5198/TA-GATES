...
def train(train_loader, model, epoch, args, arch_network_type):
    ...
    for step, train_data in enumerate(train_loader):
        ...
        acc_1 = [data[0] for data in train_data[2]]
        acc_2 = [data[1] for data in train_data[2]]
        train_accs = [np.array(acc_1), np.array(acc_2)]
...
def valid(val_loader, model, args):
    ...
    for step, (archs, zs_p, accs) in enumerate(val_loader):
        scores = model.predict((archs, zs_p), anytime=True)
        scores[0] = scores[0].cpu().data.numpy()
        scores[1] = scores[1].cpu().data.numpy()
        scores = [(scores[0][i][0], scores[1][i][0])for i in range(len(scores[0]))]
    ...
    all_scores_1 = [score[0] for score in all_scores]
    all_scores_2 = [score[1] for score in all_scores]

    true_accs_1 = [acc[0] for acc in true_accs]
    true_accs_2 = [acc[1] for acc in true_accs]

    corr_1 = stats.kendalltau(true_accs_1, all_scores_1).correlation
    corr_2 = stats.kendalltau(true_accs_2, all_scores_2).correlation
    rg_loss_1 = ((np.array(all_scores_1) - np.array(true_accs_1)) ** 2).mean()
    rg_loss_2 = ((np.array(all_scores_2) - np.array(true_accs_2)) ** 2).mean()
    
    patk_1 = p_at_tb_k(true_accs_1, all_scores_1)
    patk_2 = p_at_tb_k(true_accs_2, all_scores_2)
    natk_1 = minmax_n_at_k(true_accs_1, all_scores_1)
    natk_2 = minmax_n_at_k(true_accs_2, all_scores_2)

    patk_1 = [x[2] for x in patk_1]
    patk_2 = [x[2] for x in patk_2]
    natk_1 = [x[1] for x in natk_1]
    natk_2 = [x[1] for x in natk_2]
    
    return (corr_1, corr_2), (rg_loss_1, rg_loss_2), (patk_1, patk_2), (natk_1, natk_2)

def main(argv):
    parser = argparse.ArgumentParser(prog="train_nasbench101_pkl.py")
    ...
    setproctitle.setproctitle("python train_nasbench101_anytime.py config: {}; train_dir: {}; cwd: {}"\
    ...
    logging.info("Load pkl cache from nasbench101.pkl and nasbench101_valid.pkl")
    ...
    for i_epoch in range(1, args.epochs + 1):
        ...
        if args.eval_only_last is None or (args.epochs - i_epoch < args.eval_only_last):
            ...
            logging.info("Valid: Epoch {:3d}: kendall tau {:.4f} {:.4f}; loss {:.4g} {:.4g}".format(
                                            i_epoch, corr[0], corr[1], rg_loss[0], rg_loss[1]))
