...
from my_nas.utils import DenseGraphConvolution, DenseGraphOpEdgeFlow
...
#     from my_nas.utils import logger as _logger
...
class PointwiseComparator(ArchNetwork, nn.Module):
    ...
    def __init__(self, search_space,
        ...
        self.mlp = self.construct_mlp(dim, mlp_hiddens, mlp_dropout, mlp_out_dim)
    ...
    @staticmethod
    def construct_mlp(dim: int, mlp_hiddens: Tuple[int], mlp_dropout: float, out_dim=1) -> nn.Module:
        mlp = []
...
class AnyTimePointwiseComparator(PointwiseComparator):
    ...
    def __init__(self, search_space, seperate_mlp: bool = True,
        ...
        super(AnyTimePointwiseComparator, self).__init__(
                ...
                mlp_hiddens, mlp_dropout, mlp_out_dim, optimizer,
        ...
        if seperate_mlp:
            ...
            if hasattr(self.arch_embedder, "num_time_steps"):
                num_time_steps = self.arch_embedder.num_time_steps
            else:
                num_time_steps = 1
            self.pre_mlps = nn.ModuleList([
                    self.construct_mlp(dim, mlp_hiddens, mlp_dropout, mlp_out_dim)
    ...
    def predict(self, arch, sigmoid=True, tanh=False, anytime=False):
        if not anytime:
            ...
            last_score = self.mlp(arch_embedding_list[-1])
            last_score = torch.sigmoid(last_score)
