...
from my_nas.utils.common_utils import AverageMeter
from my_nas.utils.log import getLogger as _getLogger
from my_nas.utils.exception import expect
from my_nas.utils.lr_scheduler import get_scheduler_cls
...
class DenseGraphOpEdgeFlow_TAGATES(nn.Module):
    """
    For search space that has operation on the edge.
    """
    def __init__(self, in_features, out_features, op_emb_dim,
                 ...
                 residual_only=None, use_sum=False,
                 concat=None, has_aggregate_op=False, reverse=False,
                 skip_connection_index=None, return_message=False):
        super(DenseGraphOpEdgeFlow_TAGATES, self).__init__()
    ...
    def forward(self, inputs, adj, adj_op_inds, op_embs, self_op_emb=None):
        ...
        if self.skip_connection_index is not None:
            is_skip_mask = (adj_op_inds == self.skip_connection_index)\
        ...
        if self.residual_only is None:
            ...
            if self.reverse:
                res_output = torch.cat(
                    (torch.zeros([support.shape[0],
                                  support.shape[1] - self.residual_only, support.shape[2]],
                                 device=support.device),
                     support[:, -self.residual_only:, :]
                    ),
                    dim=1)
...
class DenseGraphSimpleOpEdgeFlow(nn.Module):
    ...
    def __init__(self, in_features, out_features, op_emb_dim,
        ...
        if skip_connection_index is not None and nonlinear == "sigmoid":
            print("Maybe specifiying skip_connection_index and using `sigmoid` "
                  "as the nonlinear is not so suitable!!!")
...
def prepare_data_queues(dataset, queue_cfg_lst, data_type="image", drop_last=False,
    ...
    if shuffle:
        if shuffle_indice_file and os.path.exists(shuffle_indice_file):
            ...
            _getLogger("my_nas.torch_utils").info(
...
def calib_bn(model, data_queue, max_inputs=2000):
    from my_nas.ops import FlexibleBatchNorm2d
    """
